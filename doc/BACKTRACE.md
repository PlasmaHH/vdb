...
* `vdb-bt-colors-rtti-warning` Sometimes gdb can't properly access the RTTI information. While we try to be as good as
  ...
  colour.
* `vdb-bt-colors-argument` Colour for function argument names.
* `vdb-bt-colors-argvalue` Colour for function argument values. Set to `None` to use default pointer coloring.
* `vdb-bt-colors-numvalue` Colour for numeric (int/float) argument values.
* `vdb-bt-colors-optimized-out` Colour for variables marked as "optimized out" by the compiler.
...
* `f` or `F` the function name. For `F` we use the full name (minus folds and shortens), for `f` we display just the
...
* `s` or `S` shows the source of that frame. Can be a source file (with line) or some object file name. For `S` we show
  the full source file path, for `s` we show just the basename.
