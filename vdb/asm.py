...
def x86_vt_flow_mov( ins, frame, possible_registers, possible_flags ):
    ...
    if( not to.memory ):
        ...
        if( frm.base != "rsp" or to.base != "rbp" ):
            frmval,_ = frm.value( possible_registers )
            possible_registers.set( to.base, frmval )
        if( frm.base == "rsp" and to.base == "rbp" ):
...
def x86_vt_flow_sub( ins, frame, possible_registers, possible_flags ):
    ...
    if( tgtv is not None and sub is not None):
        nv = tgtv - sub
        possible_registers.set( ins.arguments[1].base, nv )
...
def x86_vt_flow_xor( ins, frame, possible_registers, possible_flags ):
    ...
    if( not args[1].memory ):
        ...
        if( v0 is not None and v1 is not None ):
            t = v0 ^ v1
            possible_registers.set( args[1].base, t )
        ...
        if( len(args) == 2 ):
            ...
            if( args[0].base == args[1].base ):
                possible_registers.set( args[1].base ,0)
...
def x86_vt_flow_neg( ins, frame, possible_registers, possible_flags ):
    ...
    if( val is not None ):
        val = 0 - val
        possible_registers.set( ins.arguments[0].base, val )
        if( val == 0 ):
            possible_flags.set("CF",0)
        else:
